#!/usr/bin/env python3
# Bench de código con verificación EJECUTABLE — vía OpenRouter (cloud).
# Uso: OPENROUTER_KEY=... bench-coder-or.py <model_id> <etiqueta> <salida.md>
import json, sys, time, re, subprocess, urllib.request, os

MODEL, LABEL, OUT = sys.argv[1], sys.argv[2], sys.argv[3]
KEY = os.environ["OPENROUTER_KEY"]
API = "https://openrouter.ai/api/v1/chat/completions"

def call(prompt, maxtok=6000):
    payload = json.dumps({"model": MODEL, "temperature": 0.2, "max_tokens": maxtok,
    ...
    dt = time.time() - t0
    if "choices" not in r:
        return dt, 0, f"__NOCHOICES__ {json.dumps(r)[:200]}"
...
lines = [f"# Bench CODER (OpenRouter) — {LABEL} ({MODEL}) — {time.strftime('%H:%M')}", ""]
dt, ct, _ = call("Escribe una función Python que sume dos números. Solo el código.", 800)
...
for i, (prompt, test) in enumerate(TASKS, 1):
    ...
    if txt.startswith("__"):
        lines.append(f"C{i} [{dt:.0f}s] ❌ {txt[:100]}"); continue
    passed, err = run_test(extract_code(txt), test)
    if passed: ok += 1
